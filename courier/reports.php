	...
	<div class="card card-outline card-primary">
		<div class="card-body">
			...
			$status_arr = array("Artículo Aceptado por el Mensajero", "Recogido", "Enviado", "En Tránsito", "Llegado al Destino", "En Reparto", "Listo para Recoger", "Entregado", "Recogido", "Intento de Entrega Fallido"); ?>
                ...
                <label for="date_to" class="mx-1">Hasta</label>
                ...
                <button class="btn btn-sm btn-primary mx-1 bg-gradient-primary" type="button" id='view_report'>Ver Reporte</button>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 ">
			<div class="card">
				<div class="card-body">
					<div class="row">
						<div class="col-md-12">
        					<button type="button" class="btn btn-success float-right" style="display: none" id="print"><i class="fa fa-print"></i> Imprimir</button>
						</div>
					</div>	
...
<noscript>
	...
	<h3 class="text-center"><b>Reporte</b></h3>
</noscript>
<div class="details d-none">
		<p><b>Rango de día:</b> <span class="drange"></span></p>
		<p><b>Estado:</b> <span class="status-field">Todo</span></p>
	</div>
<script>
	function load_report(){
		...
		var status = $('#status').val()
			$.ajax({
				...
				error:err=>{
					console.log(err)
					alert_toast("Un error ha ocurrido",'error')
					end_load()
				},
...
$('#view_report').click(function(){
	if($('#date_from').val() == '' || $('#date_to').val() == ''){
		alert_toast("Seleccione primero las fechas.","error")
		return false;
	}
	load_report()
	var date_from = $('#date_from').val()
